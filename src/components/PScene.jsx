...
import { Box, Flare, Prism, Rainbow, Beam } from './forPrism'
import { calculateRefractionAngle, lerp, lerpV3 } from '../utilities/util'
import { useFrame } from '@react-three/fiber'
import { useMediaQuery } from 'react-responsive'
import { calculatePrismSizes, calculateSizes } from '../constants'
...
const PScene = ( props ) => {
  ...
  const sizes = calculatePrismSizes(isSmall, isMobile, isTablet);
  const elemSizes = calculateSizes(isSmall, isMobile, isTablet);
    ...
    <>
      ...
      {/* Caption */}
        <Center top bottom position={[0, 2, 0]}>
          <Text3D scale={sizes.captionScale} position={sizes.captionPosition} letterSpacing={-0.05} height={0.05} font="/fonts/Inter_Medium_Regular.json">
        ...
        <Beam ref={boxreflect} bounce={10} far={20}>
          <Prism position={[0, -0.5, 0]} scale={sizes.prismScale} onRayOver={rayOver} onRayOut={rayOut} onRayMove={rayMove} />
          <group name='Boxes' scale={sizes.boxGroupScale} >
        ...
        {/* <group>
              <Target position={elemSizes.targetPosition} />
              <ReactLogo position={elemSizes.reactLogoPosition} />
              <Rings position={elemSizes.ringPosition} />
              <Cube position={elemSizes.cubePosition} />
            </group> */}
