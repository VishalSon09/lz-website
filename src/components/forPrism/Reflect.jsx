...
function createEvent(api, hit, intersect, intersects) {
  if (!intersect) throw new Error('No intersect found')
...
export const Reflect = forwardRef(({ children, start: _start = [0, 0, 0], end: _end = [0, 0, 0], bounce = 10, far = 100, ...props }, fRef) => {
  ...
  const api = useMemo(
    () => ({
      ...
      positions: new Float32Array((bounce + 10) * 3),
      ...
      update: () => {
        ...
        while (true) {
          if (!api.raycaster) throw new Error('Raycaster is null')
  ...
  Reflect.displayName = 'Reflect'; // Add this line
